...
const draftsJSON = localStorage['githubDraft'] ? JSON.parse(localStorage['githubDraft']) : {}
...
const showCodeDiff = (path, fileHeader, richDiffButton, parent, iframe) => () => {
  debugger
        ...
        .then(d => {
            ...
            if (!iframe) {
              ...
              cssContentDecoded = cssFile.then(css =>  {
                ...
                codeListener = showCodeDiff(path, fileHeader, richDiffButton, parent, newIframe)
                richDiffButton.addEventListener('click', codeListener, true)
          })
            } else {
              toggleChildren(parent, fileHeader)
                richDiffButton.removeEventListener('click', diffListener, true)
                codeListener = showCodeDiff(path, fileHeader, richDiffButton, parent, iframe)
                richDiffButton.addEventListener('click', codeListener, true)
              }
...
setInterval(() => {
  ...
  if (hasStateChanged && location.pathname.match(/pull\/\d+\/files/)) {
    ...
    basePulls = getFromCache(filesUrl) || setCache(location.href, fetch(filesUrl).then(res => res.json()))
    const pull = getFromCache(pullUrl) || setCache(location.href, fetch(pullUrl).then(res => res.json()))
